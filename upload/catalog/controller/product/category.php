			...
			$results = $this->model_catalog_category->getCategories($category_id);
                        ...
                        if(!empty($results)){
                            foreach ($results as $key => $result) {
                                if($key === 0){
                                    $firstCategory = $result['category_id'];
                                }
                                $filter_data = array(
                        ...
                        $data['products'] = array();
                        if($firstCategory){
                            $data['category_id'] = $firstCategory;
                            $resultsFirstCategory = $this->model_catalog_category->getCategories($firstCategory);
                            if(!empty($resultsFirstCategory)){
                                foreach($resultsFirstCategory as $category){
                                $filter_data = array(
                                ...
                                foreach ($results as $result) {
                                    if ($result['image']) {
                                            $image = $this->model_tool_image->cropsize($result['image'], 204, 272);
                                    } else {
                                            $image = $this->model_tool_image->cropsize('placeholder.png', 204, 272);
                                    }
